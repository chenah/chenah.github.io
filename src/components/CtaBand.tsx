  ...
  return (
    <section className="grid border-b border-[#282c20] bg-[#f4f4ed] text-[#282c20] md:grid-cols-2">
      {ctas.map((cta, index) => (
        ...
        >
          <span className="text-[0.65rem] font-bold uppercase tracking-[0.2em]">{cta.kicker.en}</span>
          ...
          <div className="absolute inset-x-6 bottom-6 sm:inset-x-10 sm:bottom-10 lg:inset-x-14 lg:bottom-14">
            <span className="mb-4 block font-display text-[clamp(3.2rem,7vw,8rem)] uppercase leading-[0.8]">{t(cta.title)}</span>
            <span className="text-sm font-medium">{t(cta.desc)}</span>
          </div>
